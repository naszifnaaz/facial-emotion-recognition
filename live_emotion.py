...
def main():
    ...
    while True:
        ...
        if str(label) == "happy":
            cv2.imshow('emoji', happy)
        if str(label) == "sad":
            cv2.imshow('emoji', sad)
        if str(label) == "angry":
            cv2.imshow('emoji', angry)
        if str(label) == "neutral":
            cv2.imshow('emoji', nuetral)
        if str(label) == "surprise":
            cv2.imshow('emoji', surprise)
        if str(label) == "disgust":
            cv2.imshow('emoji', disgust)
        if str(label) == "scared":
            cv2.imshow('emoji', scared)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            break
    p = vlc.MediaPlayer("music/"+str(label)+".mp3")
    p.play()
    time.sleep(5)
    p.stop()
    print(label)
